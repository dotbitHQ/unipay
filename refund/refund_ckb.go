...
func (t *ToolRefund) doRefundCkb(list []tables.TablePaymentInfo) error {
	...
	// tx
	txBuilderBase, err := config.InitDasTxBuilderBase(t.Ctx, t.DasCore, fromCkbAddr.Script, config.Cfg.Chain.Ckb.Private)
	if err != nil {
...
func (t *ToolRefund) doRefundCkb2(paymentAddress, private string, list []tables.ViewRefundPaymentInfo) error {
	if !config.Cfg.Chain.Ckb.Refund {
		return nil
	}
	if len(list) == 0 {
	...
	// callback notice
	if err = t.addCallbackNotice2(list); err != nil {
		log.Error("addCallbackNotice err:", err.Error())
	}
	return nil
}
