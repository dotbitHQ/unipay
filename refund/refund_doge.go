...
func (t *ToolRefund) doRefundDoge2(paymentAddress, private string, list []tables.ViewRefundPaymentInfo) error {
	if !config.Cfg.Chain.Doge.Refund {
		return nil
	}
	if len(list) == 0 {
...
func (t *ToolRefund) addCallbackNotice2(list []tables.ViewRefundPaymentInfo) error {
	var noticeList []tables.TableNoticeInfo
	for _, v := range list {
		notice := tables.TableNoticeInfo{
			EventType:    tables.EventTypeOrderRefund,
